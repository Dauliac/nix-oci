...
### Debian-based E2E VM tests

End-to-end integration tests running inside Debian-based NixOS VMs to validate the full deploy lifecycle for every target:

- **NixOS deploy**: VM that builds, loads, and runs OCI containers via the `nix-oci` NixOS module, asserting service health, container metadata, and runtime behavior.
- **system-manager deploy**: VM that provisions a non-NixOS (Debian) system with [system-manager](https://github.com/numtide/system-manager), deploys containers through `nix-oci`'s system-manager module, and validates that services start, healthchecks pass, and volumes are mounted correctly.
- **home-manager deploy**: VM that runs [home-manager](https://nix-community.github.io/home-manager/) as a regular user inside a Debian guest, deploys containers via the `nix-oci` home-manager module, and asserts per-user container lifecycle (start, healthcheck, stop).
- **Compose integration**: VM tests that exercise the full `docker-compose`/`podman-compose` workflow generated by the compose module, covering multi-container stacks with networking, volume mounts, and dependency ordering.

These tests ensure the three deploy targets (NixOS, home-manager, system-manager) stay symmetric and catch regressions in the shared option tree, image loading, and service wiring.

### Cross-platform architecture VM tests

VM-based tests that validate cross-compilation and multi-arch image builds:

- **`aarch64-linux` cross-build**: build `x86_64-linux` → `aarch64-linux` images using `pkgsCross`, load them in a QEMU `aarch64` VM, and assert the container runs correctly (entrypoint, healthcheck, architecture label).
- **`aarch64-linux` emulated build**: build `aarch64-linux` images via `binfmt_misc` emulation, load them in a QEMU `aarch64` VM, and validate identical behavior to cross-built images.
- **Multi-arch manifest**: build a multi-arch OCI index (`x86_64-linux` + `aarch64-linux`), push to a local registry, pull from both architecture VMs, and assert each receives the correct platform-specific image.
- **`riscv64-linux` cross-build** (stretch goal): cross-compile a minimal image for `riscv64` and validate the image manifest metadata without a full VM (no QEMU `riscv64` VM available in NixOS tests yet).

These tests guard against cross-compilation regressions (wrong `pkgsCross` wiring, architecture label mismatches, missing `binfmt` registration) and ensure multi-arch manifests are correctly assembled.
