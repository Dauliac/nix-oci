    ...
    mkPushTempOCIApp = mkOption {
      ...
      default =
        ...
        let
          ...
          tempTagPrefix = container.multiArch.tempTagPrefix;
          baseName =
            if container.registry != null && container.registry != "" then
              "${container.registry}/${container.name}"
            else
              container.name;
        in
    ...
    mkMergeMultiArchApp = mkOption {
      ...
      default =
        ...
        let
          ...
          primaryTag = builtins.head container.tags;
          additionalTags = builtins.filter (tag: tag != primaryTag) container.tags;
        in
        pkgs.writeShellApplication {
          ...
          text = ''
            ...
            if [ -n "''${OCI_DIR:-}" ]; then
              ...
              # Build refs arguments for regctl index create
              REFS=""
              for arch in ${lib.concatStringsSep " " arches}; do
                TEMP_TAG="${container.multiArch.tempTagPrefix}-$arch-$COMMIT_SHA"
