    ...
    let
      ...
      stdoutCheck = optionalString (entry.stdout != null) ''
        ...
        assert ${pyStr entry.stdout} in stdout, (
            f"Expected stdout to contain '${lib.escape [ "'" ] entry.stdout}', got: {stdout[:500]}"
        )
      '';
    in
    ''
    ...
    let
      containsCheck = optionalString (http.contains != "") ''
        assert ${pyStr http.contains} in resp.text, (
            f"Expected response to contain '${lib.escape [ "'" ] http.contains}', got: {resp.text[:500]}"
        )
      '';
    in
    ''
    ...
    let
      checks = mapAttrsToList (key: value: ''
        assert ${pyStr key} in env_dict, (
            f"Expected env var '${lib.escape [ "'" ] key}', got keys: {list(env_dict)}"
        )
        assert ${pyStr value} in env_dict[${pyStr key}], (
            f"Expected '${lib.escape [ "'" ] key}' to contain '${lib.escape [ "'" ] value}', got: {env_dict[${pyStr key}]!r}"
        )
      '') envAttrs;
    in
    ''
