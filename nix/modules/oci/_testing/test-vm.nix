        ...
        bdd-vm = testHelpers.mkVMTest {
          ...
          testScript =
            let
              ...
              useSoci = config.test.oci._vmSociEnabled or false;
              socketUnit =
                if backend == "docker" then "docker.socket" else "podman.socket";
              containerServicePrefix =
                if backend == "docker" then "docker-" else "podman-";
              dockerHost =
            ...
            ''
              ...
              ${lib.optionalString useSoci ''
                # Verify SOCI snapshotter is running
                machine.wait_for_unit("soci-snapshotter.service")
                machine.succeed("systemctl is-active soci-snapshotter.service")
              ''}
              ...
              ${lib.optionalString useSoci ''
                # Verify SOCI was actually used for lazy pulling
                soci_log = machine.succeed(
                    "journalctl -u soci-snapshotter.service --no-pager 2>&1"
                )
                assert "snapshotter is supported" in soci_log or "soci" in soci_log.lower(), (
                    f"SOCI snapshotter does not appear to be working. Journal: {soci_log[:1000]}"
                )
              ''}
